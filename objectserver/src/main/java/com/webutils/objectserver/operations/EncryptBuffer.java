...
public class EncryptBuffer implements Operation {
     ...
     */
    private final OperationTypeEnum operationType = OperationTypeEnum.ENCRYPT_BUFFER;
    ...
    **   the ClientReadBufferManager.
    ...
    public void markRemovedFromQueue(final boolean delayedExecutionQueue) {
        ...
        if (delayedExecutionQueue) {
            LOG.warn("EncryptBuffer[" + requestContext.getRequestId() + "] markRemovedFromQueue(true) not supposed to be on delayed queue");
        } else if (onExecutionQueue){
            onExecutionQueue = false;
        } else {
            LOG.warn("EncryptBuffer[" + requestContext.getRequestId() + "] markRemovedFromQueue(false) not on a queue");
        }
    }

    public void markAddedToQueue(final boolean delayedExecutionQueue) {
        if (delayedExecutionQueue) {
            LOG.warn("EncryptBuffer[" + requestContext.getRequestId() + "] markAddToQueue(true) not supposed to be on delayed queue");
        } else {
