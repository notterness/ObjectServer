...
public class ListWorkRequestLogHandler extends SyncHandler {
    private final Authenticator authenticator;
    private final ObjectMapper mapper;
    private final WorkRequestBackend backend;
    private final EmbargoV3 embargoV3;

    public ListWorkRequestLogHandler(Authenticator authenticator, ObjectMapper mapper, WorkRequestBackend backend,
                                     EmbargoV3 embargoV3) {
    ...
    public void handleSync(RoutingContext context) {
        ...
        MetricsHandler.addMetrics(context, WebServerMetrics.V2_LIST_WORK_REQUEST_LOG_BUNDLE);
        ...
        final List<WorkRequestLog> workRequestLogJson = status.getLogs();
        ...
        if (startWith != 0 && (startWith < 0 || startWith >= workRequestLogJson.size())) {
            throw new HttpException(V2ErrorCode.INVALID_PAGE,
                    "The 'page' query parameter is not valid.", request.path());
        }
        final int limit = HttpPathQueryHelpers.getLimit(request).orElse(1000);

        List<WorkRequestLog> logCurPageList = workRequestLogJson.subList(startWith,
                Integer.min(startWith + limit, workRequestLogJson.size()));
        if (startWith + logCurPageList.size() != workRequestLogJson.size()) {
