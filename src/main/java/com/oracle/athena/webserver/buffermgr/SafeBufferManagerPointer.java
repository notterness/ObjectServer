...
/*
 ** The SafeBufferManagerPointer is used to allow multiple Producers (Operations that produce data) and multiple
 **   Consumers (Operations that perform work on data that has been produced) to use the same
 **   ThreadSafeBufferManager from different threads.
 ** The SafeBufferManagerPointer allows dependencies to be setup between Producers and Consumers. This allows
 **   multiple Consumers to be notified when a Producer has placed data into the ThreadSafeBufferManager.
 **
 ...
 **
 ** NOTE: Due to Java not supporting unsigned integer values, the size of the ThreadSafeBufferManger ring buffer
 **   must be a power of 2. This allows AND operations to be used to insure that between value accesses the
 */
public class SafeBufferManagerPointer {

    private static final Logger LOG = LoggerFactory.getLogger(SafeBufferManagerPointer.class);
     ...
     */
    SafeBufferManagerPointer ptrThisDependsOn;
    LinkedList<SafeBufferManagerPointer> dependentPointers;
     ...
     */
    SafeBufferManagerPointer(final Operation operation, final int bufferArraySize, final int identifier) {
     ...
     */
    SafeBufferManagerPointer(final Operation operation, final SafeBufferManagerPointer dependsOnPointer,
        ...
        if (dependsOnBookmark == -1) {
            this.bufferIndex.set(this.ptrThisDependsOn.getCurrIndex());
        } else {
            this.bufferIndex.set(dependsOnBookmark);

            /*
             ** Since this registration has a dependency, check if the event() for the operation should be
             **   called.
             */
            if (dependsOnBookmark != this.ptrThisDependsOn.getCurrIndex()) {
                operation.event();
            }
        }
    }
     ...
     */
    void setBookmark() {
        int pos = bufferIndex.get();

        if (ptrThisDependsOn != null) {
            LOG.error("Consumer(" + identifier + ":" + getOperationType() + ") setBookmark: " + pos + " on Producer(" +
                    ptrThisDependsOn.getIdentifier() + ":" + ptrThisDependsOn.getOperationType() + ")");

            ptrThisDependsOn.setBookmark(pos);
        } else {
            LOG.error("Producer(" + identifier + ":" + getOperationType() + ") setBookmark(1): " + pos);
    ...
    int getReadIndex(final boolean updatePointer) {
        ...
        if (ptrThisDependsOn != null) {
            if (readIndex != ptrThisDependsOn.getCurrIndex()) {
                 ...
                 */
                /*
                LOG.error("Consumer(" + identifier + ":" + getOperationType() + ") bufferIndex: " + readIndex +
             ...
             */
            ListIterator<SafeBufferManagerPointer> iter = dependentPointers.listIterator(0);

            while (iter.hasNext()) {
                SafeBufferManagerPointer consumer = iter.next();
    ...
    int updateWriteIndex() {
        int pos = bufferIndex.getAndAccumulate(bufferArraySize, computeNextIndex);

        generateDependsOnEvents();

        //LOG.info("Producer("  + identifier + ":" + getOperationType() + ") writeIndex: " + pos);
     ...
     **   than the consumer(s).
     */
    void addDependsOn(final SafeBufferManagerPointer consumerPtr) {
