...
public class NioServerHandler implements Runnable {
    ...
    /*
    ** This is called when a new server connection is made and the handling connection state needs to be
    **   associated with the
     */
        ...
        if (clientChannel != null) {
            try {
                ...
                **   method is wired through).
                 */
                if (eventPollBalancer.registerClientSocket(clientChannel) == false) {
                    try {
                        clientChannel.close();
                    } catch (IOException io_ex) {
    ...
    ** Setup the server socket channel to be ready to accept connections.
    ...
    private ServerSocketChannel setupServerChannel() {
        ...
        InetSocketAddress serverAddr = new InetSocketAddress(InetAddress.getLoopbackAddress(), tcpListenPort);
