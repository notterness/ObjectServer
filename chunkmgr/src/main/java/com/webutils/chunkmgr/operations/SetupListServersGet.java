...
public class SetupListServersGet implements Operation {

    private static final Logger LOG = LoggerFactory.getLogger(SetupListServersGet.class);
    ...
    /*
     ** This is used to setup the initial Operation dependencies required to handle the ListServers GET
     **   method.
     ** The completeCb will call the DetermineRequest operation's event() method when the GET completes.
     */
    public SetupListServersGet(final ChunkAllocRequestContext requestContext, final MemoryManager memoryManager,
        ...
        /*
         ** Setup the list of Operations currently used to handle the ListServers GET
         */
    ...
    public void execute() {
        if (!setupMethodDone) {
            /*
            ** The ListServers is dependent upon the output
             */
            ListServers listServers = new ListServers(requestContext, memoryManager, this);
            HandlerOperations.put(listServers.getOperationType(), listServers);
            BufferManagerPointer responseWritePtr = listServers.initialize();
    ...
    public void complete() {
        ...
        operation = HandlerOperations.remove(OperationTypeEnum.LIST_SERVERS);
        operation.complete();
        ...
        LOG.info("SetupListServersGet[" + requestContext.getRequestId() + "] completed");
    }
    ...
    public void markRemovedFromQueue(final boolean delayedExecutionQueue) {
        //LOG.info("SetupListServersGet[" + requestContext.getRequestId() + "] markRemovedFromQueue(" + delayedExecutionQueue + ")");
        if (delayedExecutionQueue) {
            LOG.warn("SetupListServersGet[" + requestContext.getRequestId() + "] markRemovedFromQueue(true) not supposed to be on delayed queue");
        } else if (onExecutionQueue){
            onExecutionQueue = false;
        } else {
            LOG.warn("SetupListServersGet[" + requestContext.getRequestId() + "] markRemovedFromQueue(false) not on a queue");
        }
    }

    public void markAddedToQueue(final boolean delayedExecutionQueue) {
        if (delayedExecutionQueue) {
            LOG.warn("SetupListServersGet[" + requestContext.getRequestId() + "] markAddToQueue(true) not supposed to be on delayed queue");
        } else {
    ...
    public boolean hasWaitTimeElapsed() {
        LOG.warn("SetupListServersGet[" + requestContext.getRequestId() + "] hasWaitTimeElapsed() not supposed to be on delayed queue");
        return true;
    }
