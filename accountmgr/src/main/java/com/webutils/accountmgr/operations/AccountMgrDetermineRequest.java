...
public class AccountMgrDetermineRequest implements Operation {

    private static final Logger LOG = LoggerFactory.getLogger(AccountMgrDetermineRequest.class);

    /*
     ** A unique identifier for this Operation so it can be tracked.
     */
    private final OperationTypeEnum operationType = OperationTypeEnum.ACCOUNT_MGR_DETERMINE_REQUEST;
    ...
    public AccountMgrDetermineRequest(final RequestContext requestContext, final Map<HttpMethodEnum, Operation> supportedHttpRequests) {
     ...
     */
    public void execute() {
        Operation sendFinalStatus = requestContext.getOperation(OperationTypeEnum.ACCOUNT_MGR_SEND_FINAL_STATUS);
        if (sendFinalStatus == null) {
            LOG.info("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] sendFinalStatus null");
            return;
        }
        ...
        if (!methodDeterminationDone) {
            if (requestContext.getHttpParseError()) {
                 ...
                 */
                LOG.warn("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] sending final status HTTP Parse error");
                ...
                if (httpRequestSetup != null) {
                    ...
                    LOG.info("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] execute(1) " + method.toString());
                    httpRequestSetup.initialize();
                    httpRequestSetup.event();
                } else {
                    LOG.info("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] execute(1) unsupported request " + method.toString());
            ...
            HttpMethodEnum method = httpRequestInfo.getMethod();
            LOG.info("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] execute(2) " + method.toString());
    ...
    public void markRemovedFromQueue(final boolean delayedExecutionQueue) {
        //LOG.info("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] markRemovedFromQueue(" + delayedExecutionQueue + ")");
        if (delayedExecutionQueue) {
            LOG.warn("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] markRemovedFromQueue(true) not supposed to be on delayed queue");
        } else if (onExecutionQueue){
            onExecutionQueue = false;
        } else {
            LOG.warn("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] markRemovedFromQueue(false) not on a queue");
        }
    }

    public void markAddedToQueue(final boolean delayedExecutionQueue) {
        if (delayedExecutionQueue) {
            LOG.warn("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] markAddToQueue(true) not supposed to be on delayed queue");
        } else {
    ...
    public boolean hasWaitTimeElapsed() {
        LOG.warn("AccountMgrDetermineRequest[" + requestContext.getRequestId() + "] hasWaitTimeElapsed() not supposed to be on delayed queue");
        return true;
    }
