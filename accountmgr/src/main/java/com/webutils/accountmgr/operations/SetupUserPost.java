...
public class SetupUserPost implements Operation {

    private static final Logger LOG = LoggerFactory.getLogger(SetupUserPost.class);

    /*
     ** A unique identifier for this Operation so it can be tracked.
     */
    private final OperationTypeEnum operationType = OperationTypeEnum.SETUP_USER_POST;
    ...
    private final CreateUserPostContent createUserPostContent;
    ...
    /*
     ** This is used to setup the initial Operation dependencies required to handle the CreateUser POST
     **   method.
     ** The completeCb will call the DetermineRequest operation's event() method when the POST completes.
     */
    public SetupUserPost(final AccountMgrRequestContext requestContext, final Operation metering,
        ...
        this.createUserPostContent = new CreateUserPostContent();
    ...
    public void execute() {
        if (!setupMethodDone) {
             ...
             */
            createUser = new CreateUser(requestContext, createUserPostContent, this);
            PostHandlerOperations.put(createUser.getOperationType(), createUser);
            ...
            ParseContentBuffers parseContentBuffers = new ParseContentBuffers(requestContext, readBufferPointer,
                    metering, createUserPostContent, this);
            ...
            if (updator.getSha256DigestComplete() && requestContext.postMethodContentParsed()) {
                LOG.info("SetupUserPost[" + requestContext.getRequestId() + "] Sha-256 and Parsing done");
                ...
                LOG.info("SetupUserPost[" + requestContext.getRequestId() + "] not completed Sha-256 digestComplete: " +
    ...
    public void complete() {
        ...
        LOG.info("SetupUserPost[" + requestContext.getRequestId() + "] completed");
    }
    ...
    public void markRemovedFromQueue(final boolean delayedExecutionQueue) {
        //LOG.info("SetupUserPost[" + requestContext.getRequestId() + "] markRemovedFromQueue(" + delayedExecutionQueue + ")");
        if (delayedExecutionQueue) {
            LOG.warn("SetupUserPost[" + requestContext.getRequestId() + "] markRemovedFromQueue(true) not supposed to be on delayed queue");
        } else if (onExecutionQueue){
            onExecutionQueue = false;
        } else {
            LOG.warn("SetupUserPost[" + requestContext.getRequestId() + "] markRemovedFromQueue(false) not on a queue");
        }
    }

    public void markAddedToQueue(final boolean delayedExecutionQueue) {
        if (delayedExecutionQueue) {
            LOG.warn("SetupUserPost[" + requestContext.getRequestId() + "] markAddToQueue(true) not supposed to be on delayed queue");
        } else {
    ...
    public boolean hasWaitTimeElapsed() {
        LOG.warn("SetupUserPost[" + requestContext.getRequestId() +
