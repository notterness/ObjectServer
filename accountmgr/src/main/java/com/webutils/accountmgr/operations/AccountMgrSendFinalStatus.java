...
public class AccountMgrSendFinalStatus implements Operation {


    private static final Logger LOG = LoggerFactory.getLogger(AccountMgrSendFinalStatus.class);

    /*
     ** A unique identifier for this Operation so it can be tracked.
     */
    private final OperationTypeEnum operationType = OperationTypeEnum.ACCOUNT_MGR_SEND_FINAL_STATUS;
    ...
    public AccountMgrSendFinalStatus(final RequestContext requestContext, final MemoryManager memoryManager) {
    ...
    public void execute() {
        ...
        if (!httpResponseSent) {
            ...
            if (respBuffer != null) {
                ...
                if (result != null) {
                    LOG.info("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] resultCode: " + result.getCode() + " " + result.getMessage());
                } else {
                    LOG.info("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] resultCode: INVALID (" + resultCode +")");
                }
                 ...
                 */
                LOG.info("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] unable to allocate response buffer");
            ...
            LOG.warn("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] execute() after status sent");
        }
    }
    ...
    public void complete() {
        ...
        LOG.info("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] complete() bufferCount: " + bufferCount);
        ...
        for (int i = 0; i < bufferCount; i++) {
            ...
            if (buffer != null) {
                ...
                LOG.info("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] missing buffer i: " + i);
            }
        }
    ...
    public void markRemovedFromQueue(final boolean delayedExecutionQueue) {
        //LOG.info("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] markRemovedFromQueue(" + delayedExecutionQueue + ")");
        if (delayedExecutionQueue) {
            LOG.warn("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] markRemovedFromQueue(true) not supposed to be on delayed queue");
        } else if (onExecutionQueue){
            onExecutionQueue = false;
        } else {
            LOG.warn("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] markRemovedFromQueue(false) not on a queue");
        }
    }

    public void markAddedToQueue(final boolean delayedExecutionQueue) {
        if (delayedExecutionQueue) {
            LOG.warn("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] markAddToQueue(true) not supposed to be on delayed queue");
        } else {
    ...
    public boolean hasWaitTimeElapsed() {
        LOG.warn("AccountMgrSendFinalStatus[" + requestContext.getRequestId() + "] hasWaitTimeElapsed() not supposed to be on delayed queue");
        return true;
    }
